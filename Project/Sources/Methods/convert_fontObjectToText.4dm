...
If ($Obj_in._bold#Null:C1517)
	...
	If (Length:C16($Txt_font)>0)
		...
		$Txt_font:=$Txt_font+Char:C90(Space:K15:42)
		
	End if 
	
	$Txt_font:=$Txt_font+"bold"
...
If ($Obj_in._size#Null:C1517)
	...
	$Txt_font:=$Txt_font+$Obj_in._size
...
If ($Obj_in._font#Null:C1517)
	...
	If (OB Get type:C1230($Obj_in;"_font")=Is text:K8:3)
		...
		$Txt_font:=$Txt_font+Choose:C955(Position:C15(" ";$Obj_in._font)>0;\
			"'"+$Obj_in._font+"'";$Obj_in._font)
		
		If ($Obj_in._font#"Arial")
			$Txt_font:=$Txt_font+",Arial"
		End if 
