...
Utils.addCommand('move-cells', function (params) {
	...
	if (('from' in params) && (params.from.constructor === Array)
		...
		const src = Utils.getFirstRange(params.from);
		const srcRange = [new GC.Spread.Sheets.Range(src.row, src.column, src.rowCount, src.columnCount)];

		const dest = Utils.getFirstRange(params.to);
		const destRange = [new GC.Spread.Sheets.Range(dest.row, dest.column, src.rowCount, src.columnCount)];
...
Utils.addCommand('copy-cells', function (params) {
	...
	if (('from' in params) && (params.from.constructor === Array)) {
		...
		const destRange = [new GC.Spread.Sheets.Range(src.row, src.column, src.rowCount, src.columnCount)];
...
Utils.addCommand('paste-cells', function (params) {
	...
	if (('dest' in params) && (params.dest.constructor === Array)) {
		...
		const dest = Utils.getFirstRange(params.dest);
